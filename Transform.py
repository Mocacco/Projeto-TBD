...
from Extract import extract_from_compressed
import re
...
if __name__ == "__main__":
    ...
    dados_brutos = extract_from_compressed() # Aqui você extrai os dados
    
    # 2. PASSO: CHAMAR A FUNÇÃO DE TRANSFORMAÇÃO (AQUI É ONDE OS PRINTS APARECEM)
    # Sem esta linha abaixo, o código do 'def transform...' nunca é executado!
    matriz, dados_finais = transform_com_diagnostico(dados_brutos)
    
    # 3. PASSO: CARGA (Load)
    dados_finais.to_parquet('dados_transformados.parquet')
    print("Processo finalizado e arquivo salvo!")